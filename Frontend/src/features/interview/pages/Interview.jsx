...
const Interview = () => {
    ...
    }, [ interviewId, navigate, location.state, navigationType ])
    ...
    const orderedSkillGaps = [ ...(report.skillGaps || []) ].sort((a, b) => {
        const aSeverity = (a?.severity || 'medium').toLowerCase()
        const bSeverity = (b?.severity || 'medium').toLowerCase()
        return (severityOrder[aSeverity] ?? 1) - (severityOrder[bSeverity] ?? 1)
    })
                ...
                <aside className='interview-sidebar'>
                    ...
                    <div className='skill-gaps'>
                        ...
                        <div className='skill-gaps__list'>
                            {orderedSkillGaps.map((gap, i) => {
                                const severity = (gap.severity || 'medium').toLowerCase()

                                return (
                                    <span key={i} className={`skill-tag skill-tag--${severity}`}>
                                        <span className='skill-tag__name'>{gap.skill}</span>
                                        <span className='skill-tag__severity'>({severity})</span>
                                    </span>
                                )
                            })}
                        </div>
